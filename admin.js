...
$("#loginForm").addEventListener("submit",async e=>{e.preventDefault();const f=new FormData(e.target);try{const d=await fetch("/api/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(Object.fromEntries(f))});const x=await d.json();if(!d.ok)throw new Error(x.error||"Đăng nhập thất bại");if(x.user.role!=="admin")throw new Error("Tài khoản này không phải Admin");adminToken=x.token;localStorage.setItem("gaming_admin_token",adminToken);adminUser=x.user;showAdmin()}catch(err){toast(err.message)}});
...
async function renderDesign(){settings=await api("/api/admin/settings");applyBrand();const cats=(settings.categories||[]).join(", ");const sec=settings.sections||{};const order=settings.homeOrder||["hero","products","trust"];
  ...
  <section class="design-card"><h3>Ảnh thương hiệu</h3><div class="image-grid"><div class="image-box"><strong>Logo</strong><input id="logoFile" type="file" accept="image/png,image/jpeg,image/webp,image/gif"><div id="logoPrev" class="preview">${settings.logo?`<img src="${settings.logo}" alt="logo">`:`Chưa có logo`}</div></div><div class="image-box"><strong>Favicon</strong><input id="faviconFile" type="file" accept="image/png,image/jpeg,image/webp,image/gif"><div id="faviconPrev" class="preview">${settings.favicon?`<img src="${settings.favicon}" alt="favicon">`:`Chưa có favicon`}</div></div></div></section>
  <section class="design-card"><h3>Trang chủ</h3><div class="field"><label>Dòng nhỏ<input id="heroEyebrow" value="${esc(settings.heroEyebrow)}"></label></div><div class="field"><label>Tiêu đề<input id="heroTitle" value="${esc(settings.heroTitle)}"></label></div><div class="field"><label>Mô tả<textarea id="heroSubtitle">${esc(settings.heroSubtitle)}</textarea></label></div><div class="field-grid"><div class="field"><label>Nút chính<input id="heroButtonText" value="${esc(settings.heroButtonText)}"></label></div><div class="field"><label>Nút phụ<input id="heroSecondaryText" value="${esc(settings.heroSecondaryText)}"></label></div></div><div class="field"><label>Ảnh Hero / Banner<input id="heroFile" type="file" accept="image/png,image/jpeg,image/webp,image/gif"></label><div id="heroPrev" class="preview">${settings.heroBanner?`<img src="${settings.heroBanner}" alt="hero">`:`Chưa có banner`}</div></div></section>
  <section class="design-card"><h3>Màu giao diện</h3><div class="color-grid">${colorField("primary","Màu chính",settings.primary)}${colorField("accent","Màu nhấn",settings.accent)}${colorField("pageBg","Nền trang",settings.pageBg)}${colorField("cardBg","Nền card",settings.cardBg)}${colorField("text","Màu chữ",settings.text)}${colorField("muted","Chữ phụ",settings.muted)}${colorField("footerBg","Nền footer",settings.footerBg)}</div></section>
  <section class="design-card"><h3>Danh mục</h3><div class="field"><label>Danh mục, cách nhau bằng dấu phẩy<textarea id="categories">${esc(cats)}</textarea></label><div class="helper">Ví dụ: Roblox, Currency, Items, Services, Accounts</div></div></section>
  <section class="design-card"><h3>Liên hệ</h3><div class="field-grid"><div class="field"><label>Email<input id="supportEmail" value="${esc(settings.supportEmail)}"></label></div><div class="field"><label>Số điện thoại<input id="supportPhone" value="${esc(settings.supportPhone)}"></label></div><div class="field"><label>Discord<input id="discord" value="${esc(settings.discord)}"></label></div><div class="field"><label>Facebook<input id="facebook" value="${esc(settings.facebook)}"></label></div></div><div class="field"><label>Telegram<input id="telegram" value="${esc(settings.telegram)}"></label></div></section>
  <section class="design-card"><h3>Bố cục trang chủ</h3><div class="section-toggles"><div class="toggle-row"><div><b>Hero</b><div class="row-sub">Phần giới thiệu/banner đầu trang</div></div><button class="toggle ${sec.hero!==false?"on":""}" data-section="hero" onclick="toggleSection(this)"><i></i></button></div><div class="toggle-row"><div><b>Sản phẩm</b><div class="row-sub">Catalog sản phẩm</div></div><button class="toggle ${sec.products!==false?"on":""}" data-section="products" onclick="toggleSection(this)"><i></i></button></div><div class="toggle-row"><div><b>Trust cards</b><div class="row-sub">Các khối giới thiệu / lợi ích</div></div><button class="toggle ${sec.trust!==false?"on":""}" data-section="trust" onclick="toggleSection(this)"><i></i></button></div></div><div class="order-builder" style="margin-top:12px" id="orderBuilder">${order.map((k,i)=>orderItem(k,i,order.length)).join("")}</div></section>
  <div class="form-actions"><span class="muted">Thay đổi sẽ áp dụng cho shop ngay sau khi lưu.</span><button class="btn primary" onclick="saveDesign(event)">Lưu toàn bộ giao diện</button></div>
  </div><aside class="preview-panel"><div class="design-card"><h3>Preview</h3><p class="muted">Xem nhanh bố cục hiện tại.</p><div id="sitePreview" class="site-preview"></div></div></aside></div>`;
...
function colorField(name,label,value){return `<label class="color-field"><span>${esc(label)}</span><input id="c_${name}" type="color" value="${esc(value||"#ffffff")}" oninput="updatePreview()"></label>`}
...
function orderItem(k,i,len){const label={hero:"Hero",products:"Sản phẩm",trust:"Trust cards"}[k]||k;return `<div class="order-item" data-order="${k}"><div><b>${label}</b></div><div><button type="button" onclick="moveOrder(this,-1)" ${i===0?"disabled":""}>↑</button><button type="button" onclick="moveOrder(this,1)" ${i===len-1?"disabled":""}>↓</button></div></div>`}
function moveOrder(btn,delta){const item=btn.closest(".order-item"),parent=item.parentElement;const all=[...parent.children];const i=all.indexOf(item);const j=i+delta;if(j<0||j>=all.length)return;if(delta<0)parent.insertBefore(item,all[j]);else parent.insertBefore(all[j],item);[...parent.children].forEach((el,i)=>el.querySelectorAll("button").forEach((b,n)=>b.disabled=n===0?i===0:i===parent.children.length-1));updatePreview()}
function currentOrder(){return [...document.querySelectorAll("#orderBuilder .order-item")].map(x=>x.dataset.order)}
function updatePreview(){if(!$("#sitePreview"))return;const pv={primary:$("#c_primary")?.value||settings.primary,accent:$("#c_accent")?.value||settings.accent,card:$("#c_cardBg")?.value||settings.cardBg,bg:$("#c_pageBg")?.value||settings.pageBg};const heroEnabled=document.querySelector('[data-section="hero"]')?.classList.contains("on")!==false;const productsEnabled=document.querySelector('[data-section="products"]')?.classList.contains("on")!==false;const trustEnabled=document.querySelector('[data-section="trust"]')?.classList.contains("on")!==false;const title=$("#heroTitle")?.value||settings.heroTitle;const desc=$("#heroSubtitle")?.value||settings.heroSubtitle;const heroImg=$("#heroPrev")?.dataset.image||settings.heroBanner||"";let html="";for(const k of currentOrder()){if(k==="hero"&&heroEnabled)html+=`<div class="preview-hero" style="--pv-primary:${pv.primary};--pv-accent:${pv.accent};${heroImg?`background-image:linear-gradient(90deg,rgba(9,12,18,.58),rgba(9,12,18,.15)),url('${heroImg.replace(/'/g,"%27")}')`:""}"><small>${esc($("#heroEyebrow")?.value||settings.heroEyebrow)}</small><h4>${esc(title)}</h4><p>${esc(desc)}</p></div>`;if(k==="products"&&productsEnabled){const ps=products.slice(0,4);html+=`<div class="preview-products" style="background:${pv.bg}"><div class="pv-cards">${ps.map(p=>`<div class="pv-card" style="background:${pv.card}"><div class="pv-img">${p.image?`<img src="${p.image}" alt="">`:""}</div><b>${esc(p.name)}</b><div class="pv-price" style="color:${pv.accent}">${money(p.price)}</div></div>`).join("")||`<div class="muted">Thêm sản phẩm để xem preview.</div>`}</div></div>`}if(k==="trust"&&trustEnabled)html+=`<div class="preview-trust"><div class="pv-trust">Ảnh sản phẩm rõ ràng</div><div class="pv-trust">Shop theo thương hiệu của bạn</div><div class="pv-trust">Quản lý ngay trên web</div></div>`}$("#sitePreview").innerHTML=html||`<div class="empty">Chưa bật section nào.</div>`}
async function saveDesign(e){e.preventDefault();try{const body={shopName:$("#shopName").value,shopTagline:$("#shopTagline").value,metaDescription:$("#metaDescription").value,heroEyebrow:$("#heroEyebrow").value,heroTitle:$("#heroTitle").value,heroSubtitle:$("#heroSubtitle").value,heroButtonText:$("#heroButtonText").value,heroSecondaryText:$("#heroSecondaryText").value,supportEmail:$("#supportEmail").value,supportPhone:$("#supportPhone").value,discord:$("#discord").value,facebook:$("#facebook").value,telegram:$("#telegram").value,categories:$("#categories").value.split(",").map(x=>x.trim()).filter(Boolean),sections:{hero:document.querySelector('[data-section="hero"]').classList.contains("on"),products:document.querySelector('[data-section="products"]').classList.contains("on"),trust:document.querySelector('[data-section="trust"]').classList.contains("on")},homeOrder:currentOrder()};for(const [input,prev,key] of [["logoFile","logoPrev","logo"],["faviconFile","faviconPrev","favicon"],["heroFile","heroPrev","heroBanner"]]){const el=$("#"+prev),file=$("#"+input)?.files?.[0];body[key]=file?await fileToData(file):(el?.dataset.image||settings[key]||"")}for(const k of ["primary","accent","pageBg","cardBg","text","muted","footerBg"])body[k]=$("#c_"+k)?.value||settings[k];settings=await api("/api/admin/settings",{method:"PUT",body:JSON.stringify(body)});applyBrand();toast("Đã lưu toàn bộ giao diện");setTimeout(()=>go("design"),250)}catch(err){toast(err.message)}}
boot();
